...
//比较三个迭代器，返回最大的那个迭代器指向的值，即为在未来最长时间不被访问的页面号
...
//比较物理块中的页面哪一个在未来最长时间不被访问，返回被换出的页面号
void removePage(const vector<int>::iterator it) {
	//获取物理块中的第一个页面号在当前页面号引用串中第一次出现的位置，剩下的迭代器以此类推
	vector<int>::iterator it1 = find(it, pages.end(), frames[0]);
	vector<int>::iterator it2 = find(it, pages.end(), frames[1]);
	vector<int>::iterator it3 = find(it, pages.end(), frames[2]);
	//得到三个迭代器中最大的那个迭代器，即为在未来最长时间不被访问的页面号的迭代器
	int page = compare(it1, it2, it3);
	//在物理块中找到被换出的页面号的迭代器
	vector<int>::iterator remove = find(frames.begin(), frames.end(), page);	
	removePages.push_back(page);	//把被换出的页面号放进被换出的页面号数组
...
void Optimal() {
	...
	for (; it != pages.end(); ++it) {
		...
		//如果当前的页面号不在物理块中，而且当前物理块满了，那么就要进行页面置换
			...
			removePage(it);						//调用函数，找到物理块中的页面哪一个在未来最长时间不被访问,得到被换出的页面号
